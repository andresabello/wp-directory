...
<div class="comments-area" id="comments">
	<?php if ( have_comments() ) : ?>
		<h2 class="comments-title">
			<?php 
				printf( _nx( 'One comment', '%1$s comments', get_comments_number(), 'Comment title', 'pidirectory' ), number_format_i18n( get_comments_number() ) );
			?>
		</h2>
		...
		<nav class="comment-nav" role="navigation">
			<p class="comment-nav-prev">
				<?php previous_comments_link( __( '&larr; Older Comments', 'pidirectory' ) ); ?>
			</p>

			<p class="comment-nav-next">
				<?php next_comments_link( __( 'Newer Comments &rarr;', 'pidirectory' ) ); ?>
			</p>
		</nav> <!-- end comment-nav -->
		<?php endif; ?>
		...
		?>
			<p class="no-comments">
				<?php _e( 'Comments are closed.', 'pidirectory' ); ?>
			</p>
		<?php endif; ?>
	<?php endif; ?>
